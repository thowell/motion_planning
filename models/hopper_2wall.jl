...
# Kinematics
kinematics(::Hopper2Wall, q) = [q[1] + q[4] * sin(q[3]), q[2] - q[4] * cos(q[3])]

# Methods
M_func(model::Hopper2Wall, q) = Diagonal(@SVector [
								 model.mb + model.ml,
								 model.mb + model.ml,
								 model.Jb + model.Jl,
								 model.ml])

G_func(model::Hopper2Wall, q) = @SVector [0.0,
									 (model.mb + model.ml) * model.g,
									 0.0,
									 0.0]

function ϕ_func(::Hopper2Wall, q)
	r_wall = 0.5
	l_wall = -0.5
    @SVector [q[2] - q[4] * cos(q[3]),
			  r_wall - (q[1] + q[4] * sin(q[3])),
			  q[1] + q[4] * sin(q[3]) - l_wall]
end

N_func(::Hopper2Wall, q) = @SMatrix [0.0 1.0 (q[4] * sin(q[3])) (-1.0 * cos(q[3]));
									 -1.0 0.0 (-1.0 * q[4] * cos(q[3])) -1.0 *sin(q[3]);
									 1.0 0.0 (q[4] * cos(q[3])) sin(q[3])]


function P_func(::Hopper2Wall, q)
...
B_func(::Hopper2Wall, q) = @SMatrix [0.0 0.0 1.0 0.0;
...
function fd(model::Hopper2Wall, x⁺, x, u, w, h, t)
    ...
    + transpose(P_func(model, q3)) * SVector{6}(b)
    - h * G_func(model, q2⁺))]
end

function maximum_dissipation(model::Hopper2Wall, x⁺, u, h)
	q3 = x⁺[model.nq .+ (1:model.nq)]
	q2 = x⁺[1:model.nq]
	ψ = u[model.idx_ψ]
	ψ_stack = [ψ[1] * ones(2); ψ[2] * ones(2); ψ[3] * ones(2)]
	η = u[model.idx_η]
	return P_func(model, q3) * (q3 - q2) / h + ψ_stack - η
end

function friction_cone(model::Hopper2Wall, u)
	λ = u[model.idx_λ]
	b = u[model.idx_b]
...
# Visualization
function visualize!(vis, model::Hopper2Wall, q; Δt = 0.1)
    r_foot = 0.05
    r_leg = 0.5 * r_foot

    setobject!(vis["body"], GeometryTypes.Sphere(Point3f0(0),
        convert(Float32, 0.1)),
        MeshPhongMaterial(color = RGBA(0, 1, 0, 1.0)))

    setobject!(vis["foot"], GeometryTypes.Sphere(Point3f0(0),
        convert(Float32, r_foot)),
        MeshPhongMaterial(color = RGBA(1, 0, 0, 1.0)))

    n_leg = 100
    for i = 1:n_leg
        setobject!(vis["leg$i"], GeometryTypes.Sphere(Point3f0(0),
