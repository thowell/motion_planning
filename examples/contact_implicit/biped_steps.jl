...
obj_control = quadratic_tracking_objective(
    ...
    [[copy(u1); zeros(model.m - model.nu)] for t = 1:T-1])
...
l_terminal_fh1(x) = 0.0 * (kinematics_2(model, view(x, 8:14), body = :leg_1, mode = :ee)[2])^2.0
...
l_terminal_fh2(x) = 0.0 * (kinematics_2(model, view(x, 8:14), body = :leg_2, mode = :ee)[2])^2.0
