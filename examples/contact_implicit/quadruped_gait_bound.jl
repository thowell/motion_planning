...
xr2 = [[xr2_el[1] for t = 1:Tm-1 + T_fix]..., xr2_el...]
zr2 = [[zr2_el[1] for t = 1:Tm-1 + T_fix]..., zr2_el...]
pr2_ref = [[xr2[t]; zr2[t]] for t = 1:T]


xf1_el, zf1_el = ellipse_traj(pf1[1], pf1[1] + strd, zh, Tm - T_fix)
xf1 = [[xf1_el[1] for t = 1:T_fix]..., xf1_el..., [xf1_el[end] for t = 1:Tm-1]...]
zf1 = [[zf1_el[1] for t = 1:T_fix]..., zf1_el..., [zf1_el[end] for t = 1:Tm-1]...]
...
function l_stage(x, u, t)
    ...
    if true##t >= Tm || t < T_fix + 1
		...
		J += 10000.0 * sum((pr1_ref[t] - kinematics_2(model, q1, body = :calf_1, mode = :ee)).^2.0)
    ...
    if true ##t <= Tm + T_fix || t == T
		...
		J += 10000.0 * sum((pf1_ref[t] - kinematics_3(model, q1, body = :calf_3, mode = :ee)).^2.0)
...
function pinned1!(c, x, u, t)
	...
	c[3:4] = pr2_ref[t] - kinematics_2(model, q, body = :calf_2, mode = :ee)
...
function pinned2!(c, x, u, t)
	...
	c[1:2] = pf1_ref[t] - kinematics_3(model, q, body = :calf_3, mode = :ee)
