...
q1, qM, qT = initial_configuration_1(model, pi - pi / 50.0, pi / 10.0, -pi / 20.0)
...
foot_z1 = sqrt.((zh^2.0) * (1.0 .- ((foot_2_x1).^2.0) ./ abs(foot_2_1[1])^2.0))

foot_1_x2 = range(foot_1_M[1], stop = foot_1_T[1], length = 26)
foot_z2 = sqrt.((zh^2.0) * (1.0 .- ((foot_1_x2 .- abs(foot_1_T[1] / 2.0)).^2.0) ./ abs(foot_1_T[1] / 2.0)^2.0))
using Plots
...
_uu[model.idx_u] .= 100.0 # model.uU
_uu[end] = 2.0 * h
_ul = zeros(model.m)
_ul[model.idx_u] .= -100.0 # model.uL
_ul[end] = 0.5 * h
...
xl[26] = [-Inf * ones(model.nq); qM]
xu[26] = [Inf * ones(model.nq); qM]
...
obj_control = quadratic_time_tracking_objective(
    ...
    [Diagonal([1.0e-3 * ones(model.nu)..., 1.0e-3 * ones(model.m - model.nu - 1)..., 0.0]) for t = 1:T-1],
    [x0[t] for t = 1:T],
...
obj_velocity = velocity_objective(
    [Diagonal(5.0 * ones(model.nq)) for t = 1:T-1],
    model.nq,
    h = h,
...
# torso lateral
l_stage_torso_lat(x, u, t) = (0.0 * (kinematics_1(model, view(x, 8:14), body = :torso, mode = :com)[1] - kinematics_1(model, view(x0[t], 8:14), body = :torso, mode = :com)[1])^2.0)
