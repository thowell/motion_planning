...
x1 = zeros(models.n[1])
x1_add = [0.0, 0.0, 0.0]#, 0.0]
for i = 1:N
	x1[(i - 1) * (model.n) + 1] = 1.0 + x1_add[i]
end
...
# Objective
Q = [(t < T ?
	 Diagonal(vcat([[1.0; 1.0] for i = 1:N]..., 1.0e-6 * ones(t == 1 ? 0 : models.p)))
	: Diagonal(vcat([[1.0; 1.0] for i = 1:N]..., 1.0e-6 * ones(t == 1 ? 0 : models.p)))) for t = 1:T]
q = [-2.0 * Q[t] * xT[t] for t = 1:T]

_R = 1.0e-1 * ones(models.m[2])
R = [Diagonal(t == 1 ? [_R; 10.0 * ones(models.p)] : _R) for t = 1:T-1]
...
p_con = [t == T ? 1 * ns : (ms + 0 * 2 * ms) for t = 1:T]
ul = [-Inf]
uu = [Inf]
...
function c!(c, cons::StageConstraints, x, u, t)
	...
	if t == T
		for i = 1:N
			c[(i - 1) * n .+ (1:n)] .= view(x, (i - 1) * n .+ (1:n)) - (cons.con[T].info[:xT] .+ 0.0e-3)
