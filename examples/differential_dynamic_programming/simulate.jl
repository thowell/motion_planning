...
function simulate_linear_feedback(
    ...
    for tt = 1:T_sim-1
        ...
        J += (x_sim[end] - x_cubic)' * Q[k + 1] * (x_sim[end] - x_cubic)
        J += (u_sim[end] - ū[k])' * R[k] * (u_sim[end] - ū[k])
        Jx += (x_sim[end] - x_cubic)' * Q[k + 1] * (x_sim[end] - x_cubic)
        Ju += (u_sim[end] - ū[k])' * R[k] * (u_sim[end] - ū[k])
    end
