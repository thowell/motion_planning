...
function fd(model::Cartpole{Midpoint, FixedTime}, x, u, w, h, t)
...
function fdx(model::Cartpole{Midpoint, FixedTime}, x, u, w, h, t)
	rz(z) = fd(model, z, x, u, w, h, t) # implicit Euler integration
    x⁺ = newton(rz, copy(x))
    # x⁺ = levenberg_marquardt(rz, copy(x))
...
function fdu(model::Cartpole{Midpoint, FixedTime}, x, u, w, h, t)
	rz(z) = fd(model, z, x, u, w, h, t) # implicit Euler integration
    x⁺ = newton(rz, copy(x))
    # x⁺ = levenberg_marquardt(rz, copy(x))
	∇rz = ForwardDiff.jacobian(rz, x⁺)
