...
obj = quadratic_tracking_objective(
        [t < T ? Diagonal(1.0e-2 * ones(model.n)) : 10.0 * Diagonal(ones(model.n)) for t = 1:T],
        [Diagonal(1.0e-3 * ones(model.m)) for t = 1:T-1],
        [[1.0; 2.0; 0.0; 1.0; 2.0; 0.0] for t = 1:T],
...
x0 = configuration_to_state(linear_interpolation(q1, [1.0; 2.0; 0.0], T)) #linear_interpolation(x1, x1, T) # linear interpolation on state
