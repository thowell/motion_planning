...
# Free-time model
model_ft_nom = free_time_model(model_nominal)
...
# Bounds
ul, uu = control_bounds(model_ft_nom, T, [-5.0; 0.0; 0.01], [5.0; 100.0; 1.0])

# Initial and final states
x1 = [15.0;
	  model_ft_nom.l1 + 10.0;
	  -45.0 * pi / 180.0;
	  -10.0;
	  -10.0;
	  -1.0 * pi / 180.0]
...
# xl <= x <= xl
_xl = -Inf * ones(model_ft_nom.n)
_xl[2] = model_ft_nom.l1
_xu = Inf * ones(model_ft_nom.n)

xl, xu = state_bounds(model_ft_nom, T, _xl, _xu, x1 = x1, xT = xT)
...
obj = quadratic_time_tracking_objective(
    ...
    [xT for t = 1:T],
	[zeros(model_ft_nom.m) for t = 1:T],
	1.0)
...
U0 = [ones(model_ft_nom.m) for t = 1:T-1] # random controls
