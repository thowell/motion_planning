...
struct ContactALConstraints <: Constraints
	n
	ineq
end

function contact_al_constraints(model, T)
	n = model.nc * (T + 1) + model.nc * (T - 1) + model.nb * (T - 1) + 2 * model.nc * (T - 1) + model.nb * (T - 1)
	ineq = con_ineq_contact(model, T)

	return ContactALConstraints(n, ineq)
end

function constraints!(c, Z, con::ContactALConstraints, model, idx, h, T)
	shift = 0
	...
	for t = 1:T-1
		...
		c[shift + (t-1) * model.nc .+ (1:model.nc)] = min.(λ, ϕ_func(model, q)) #λ .* ϕ_func(model, q)
	end
	...
	for t = 1:T-1
		...
		c[shift + (t-1) * model.nc .+ (1:model.nc)] = min.(ψ, friction_cone(model, u)) #ψ .* friction_cone(model, u) #
	end
	...
	for t = 1:T-1
		...
		c[shift + (t-1) * model.nb .+ (1:model.nb)] = min.(η, b) # η .* b #
	end
...
function constraints_jacobian!(∇c, Z, con::ContactALConstraints, model, idx, h, T)
	shift = 0
	c_shift = 0
	...
	for t = 1:T-1
		...
		cq(y) = min.(λ, ϕ_func(model, y)) # λ .* ϕ_func(model, y) #
		cu(y) = min.(y[model.idx_λ], ϕ_func(model, q)) # y[model.idx_λ] .* ϕ_func(model, q) #
	...
	for t = 1:T-1
		...
		cu(y) = min.(y[model.idx_ψ], friction_cone(model, y)) # y[model.idx_ψ] .* friction_cone(model, y) #
	...
	for t = 1:T-1
		...
		cu(y) = min.(y[model.idx_η], y[model.idx_b]) #y[model.idx_η] .* y[model.idx_b] #
...
function constraints_sparsity(con::ContactALConstraints, model, idx, T;
